...
class DGSRCA(nn.Module):
    def __init__(self, user_num, item_num, input_dim, item_max_length, user_max_length, feat_drop=0.2, attn_drop=0.2,
        ...
        if self.community_num is not None:
            ...
            # Gating Network: Decides the "weight" of the community (Scalar 0-1)
            # Input: [Node Emb || Community Emb] -> Output: 1 value (alpha)
            # We use separate gates for items and users as their distributions differ
            self.item_gate_layer = nn.Linear(self.hidden_size * 2, 1, bias=True)
            self.user_gate_layer = nn.Linear(self.hidden_size * 2, 1, bias=True)
            
            # High dropout on community features to prevent overfitting (model shouldn't rely 100% on it)
            self.comm_dropout = nn.Dropout(0.5)
        # ------------------------------------
    ...
    def forward(self, g, user_index=None, last_item_index=None, neg_tar=None, is_training=False):
        ...
        # --- Communities as a Gate ---

        # 1. Initialize User Embedding
        user_h = self.user_embedding(g.nodes['user'].data['user_id'].cuda())
        
        # --- USER GATING LOGIC ---
        if self.community_num is not None and 'community_id' in g.nodes['user'].data:
            # Get User Communities
            u_comm_ids = g.nodes['user'].data['community_id'].cuda()
            u_comm_emb = self.community_embedding(u_comm_ids)
            u_comm_emb = self.comm_dropout(u_comm_emb) # Apply Dropout

            # Calculate Alpha (Gating Coefficient) for User
            cat_input_u = torch.cat([user_h, u_comm_emb], dim=-1)
            alpha_u = torch.sigmoid(self.user_gate_layer(cat_input_u))
            
            # Weighted Sum: New User = User + (Alpha * Community)
            user_h = user_h + (alpha_u * u_comm_emb)
        # -------------------------
        
        g.nodes['user'].data['user_h'] = user_h
        
        # 2. Initialize Item Embedding
        item_h = self.item_embedding(g.nodes['item'].data['item_id'].cuda())

        # --- ITEM GATING LOGIC ---
        if self.community_num is not None and 'community_id' in g.nodes['item'].data:
            # Get Item Communities
            i_comm_ids = g.nodes['item'].data['community_id'].cuda()
            i_comm_emb = self.community_embedding(i_comm_ids)
            i_comm_emb = self.comm_dropout(i_comm_emb)

            # Calculate Alpha (Gating Coefficient) for Item
            cat_input_i = torch.cat([item_h, i_comm_emb], dim=-1)
            alpha_i = torch.sigmoid(self.item_gate_layer(cat_input_i))

            # Weighted Sum: New Item = Item + (Alpha * Community)
            item_h = item_h + (alpha_i * i_comm_emb)
        # -------------------------

        g.nodes['item'].data['item_h'] = item_h
...
def neg_generate(user, data_neg, neg_num=100):
    ...
    for i, u in enumerate(user):
        # Convert tensor to a standard python int
        u_idx = u.item() if hasattr(u, 'item') else u
        neg[i] = np.random.choice(data_neg[u_idx], neg_num, replace=False)
    return neg
