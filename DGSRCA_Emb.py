...
class DGSRCA(nn.Module):
    def __init__(self, user_num, item_num, input_dim, item_max_length, user_max_length, feat_drop=0.2, attn_drop=0.2,
        ...
        if self.community_num is not None:
            ...
            self.comm_map = nn.Linear(self.hidden_size * 2, self.hidden_size, bias=False)
        # ---------------------------------------
    ...
    def forward(self, g, user_index=None, last_item_index=None, neg_tar=None, is_training=False):
        ...
        # --- Communities as an Embedding ---
        if self.community_num is not None and 'community_id' in g.nodes['item'].data:
            comm_ids = g.nodes['item'].data['community_id'].cuda()
            comm_emb = self.community_embedding(comm_ids)
            
            # CONCATENA: [Item Embedding || Community Embedding]
            # Dimensão resultante: hidden_size * 2
            item_cat = torch.cat([g.nodes['item'].data['item_h'], comm_emb], dim=-1)
            
            # PROJETA: Reduz de volta para hidden_size para entrar na GNN
            g.nodes['item'].data['item_h'] = self.comm_map(item_cat)
        # -----------------------------------------
